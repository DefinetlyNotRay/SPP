   ...
   Object selectedNisnObject = jComboNisn.getSelectedItem();
    ...
    private void jToggleButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButton1ActionPerformed
           try {
                ...
                for (int row = 0; row < tabelAkun1.getRowCount(); row++) {
                    for (int col = 0; col < tabelAkun1.getColumnCount(); col++) {
                        document.add(new Paragraph(tabelAkun1.getValueAt(row, col).toString()));
                    }
                }
